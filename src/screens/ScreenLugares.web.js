...
import Lugares from '../components/locations/MostrarLugares.web.js';
import { fetchAllLocationFilters, fetchLocationsPage } from '../api/locations';
import '../styles/screenLugares.css';
...
function ScreenLugares() {
  ...
  useEffect(() => {
    const loadFilters = async () => {
      try {
        const { types, dimensions, relations } =
          await fetchAllLocationFilters();

        const uniqueTypes = [...new Set(types)].sort((a, b) =>
          a.localeCompare(b)
        );
        const uniqueDims = [...new Set(dimensions)].sort((a, b) =>
          a.localeCompare(b)
        );
        ...
        Object.keys(t2d).forEach(
          (t) => (t2d[t] = [...t2d[t]].sort((a, b) => a.localeCompare(b)))
        );
        Object.keys(d2t).forEach(
          (d) => (d2t[d] = [...d2t[d]].sort((a, b) => a.localeCompare(b)))
        );
  ...
  useEffect(() => {
    setDimensionOptions(
      typeFilter ? typeToDimensions[typeFilter] || [] : allDimensions
    );
    setTypeOptions(
      dimensionFilter ? dimensionToTypes[dimensionFilter] || [] : allTypes
    );
  }, [
    typeFilter,
    dimensionFilter,
  ...
  const lastLugarElementRef = useCallback(
    (node) => {
      if (loading) return;

...
import Personajes from '../components/characters/MostrarPersonajes';
import '../styles/screenPersonajes.css';
...
function ScreenPersonajes() {
  ...
  const displayOriginOptions =
    filters.origin && !originOptions.includes(filters.origin)
      ? [filters.origin, ...originOptions]
      : originOptions;

  const displayLocationOptions =
    filters.location && !locationOptions.includes(filters.location)
